...
### Added
- **Cost tracking per agent run** - prompt/completion token totals with USD cost shown after each run (servers that report usage).
- Cost calculated from provider-specific pricing (OpenAI, Anthropic, Google, local models free).
- Cost displayed in chat panel after each run: "tokens: X in · Y out · ~$Z.ZZZZ"

### Changed
- Cost tracking built on token usage feature; accumulates across multi-turn runs.
